...
class TestIfCurExpSingleNeuron(unittest.TestCase):
    ...
    def simulate(spinnaker, simtime, input_spike_times):
        # Import correct simulator
        if spinnaker:
    ...
    def plot_raster(trace, axis, offset, label, colour):
        if trace is not None and len(trace) > 0:
            ...
            axis.scatter(trace_unzipped[1], trace_unzipped[0], color=colour, label=label, s=4)

    def test_single_neuron(self):
        # Generate poisson noise
        # **YUCK** remove duplicates as SpiNNaker implementation of spike source array can only send one spike/neuron/ms
        noise_spike_times = self.poisson_generator(noise_rate, 0, simtime)
        noise_spike_times = list(collections.OrderedDict.fromkeys(noise_spike_times))

        # Simulate using both simulators
        s_pop_voltages, s_pop_spikes = self.simulate(True, simtime, noise_spike_times)
        n_pop_voltages, n_pop_spikes = self.simulate(False, simtime, noise_spike_times)
