...
//! the configuration parameters set by the host
typedef struct motor_control_parameters_t {
    uint32_t key;
    uint32_t speed;
...
static motor_control_parameters_t config;
...
static inline void send(uint32_t direction, uint32_t speed) {
    uint32_t direction_key = direction | config.key;
    ...
    if (config.delay_time > 0) {
        spin1_delay_us(config.delay_time);
    }
}
...
static inline void do_motion(
    ...
    if (delta >= config.delta_threshold) {
        ...
        last_speed[direction_index - 1] = config.speed;
        last_speed[opposite_index - 1] = 0;
        send(direction_index, config.speed);
    } else if (delta <= -config.delta_threshold) {
        log_debug("Moving %s", direction);
        last_speed[direction_index - 1] = 0;
        last_speed[opposite_index - 1] = config.speed;
        send(opposite_index, config.speed);
    } else if (!config.continue_if_not_different) {
...
static void timer_callback(uint unused0, uint unused1) {
    ...
    } else if ((time % config.update_time) == 0) {
        // Do updates in pairs
...
static void read_parameters(address_t region_address) {
    ...
    log_info("Key = %d, speed = %d, sample_time = %d, update_time = %d,"
            ...
            config.key, config.speed, config.sample_time, config.update_time,
            config.delay_time, config.delta_threshold,
            config.continue_if_not_different);
}
