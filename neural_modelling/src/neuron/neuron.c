...
bool neuron_initialise(address_t address, uint32_t recording_flags_param,
    ...
    *incoming_spike_buffer_size = address[INCOMING_SPIKE_BUFFER_SIZE];
    ...
    // I think here is where the implementation initialise function should be called
...
void neuron_do_timestep_update(timer_t time) {
    ...
    for (index_t neuron_index = 0; neuron_index < n_neurons; neuron_index++) {
    	...
    	// everything in this loop up to the state update function now goes
    	// into the implementation function instead ?
...
//
    ...
    // probably sensible to have another implementation function here
