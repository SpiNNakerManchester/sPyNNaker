...
static inline final_state_t plasticity_update_synapse(
        ...
        const post_event_history_t *events) {
    ...
    post_event_window_t post_window = post_events_get_window_delayed(
            events, window_begin_time, window_end_time);
    ...
    current_state = timing_apply_pre_spike(
            delayed_pre_time, new_pre_trace, delayed_last_pre_time,
            last_pre_trace, post_window.prev_time, post_window.prev_trace,
            current_state);
...
//---------------------------------------
struct plastic_region_t {
    pre_event_history_t history;
    plastic_synapse_t synapses[];
};

static inline plastic_synapse_t *plastic_synapses(
        void *plastic_region_address) {
    struct plastic_region_t *plastic_region = plastic_region_address;
    return plastic_region->synapses;
}
...
address_t synapse_dynamics_initialise(
        ...
        uint32_t *ring_buffer_to_input_buffer) {
    // Load timing dependence data
    address_t weight_region = timing_initialise(address);
    if (address == NULL) {
        return NULL;
    }
    ...
    address_t weight_result = weight_initialise(
            weight_region, n_synapse_types, ring_buffer_to_input_buffer);
...
bool synapse_dynamics_process_plastic_synapses(
        address_t plastic_region, address_t fixed_region,
        weight_t *ring_buffers, uint32_t time) {

    // Extract separate arrays of plastic synapses (from plastic region),
    // Control words (from fixed region) and number of plastic synapses
    plastic_synapse_t *plastic_words = plastic_synapses(plastic_region);
    ...
    pre_event_history_t *event_history = plastic_event_history(plastic_region);
