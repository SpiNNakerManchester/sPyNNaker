...
static inline pre_trace_t timing_add_pre_spike(uint32_t time,
	...
	if (!is_readout) {
		if (print_plasticity){
			io_printf(IO_BUF, "this is NOT a readout neuron!!\n");
		}
		...
		p_j = 1.0k; // this is a readout neuron

		if (print_plasticity) {
			io_printf(IO_BUF, "this IS a readout neuron!!n");
		}
	}
...
//---------------------------------------
static inline update_state_t timing_apply_post_spike(uint32_t time,
		post_trace_t trace, uint32_t last_pre_time, pre_trace_t last_pre_trace,
		uint32_t last_post_time, post_trace_t last_post_trace,
		update_state_t previous_state) {
	...
	if (time_since_last_pre > 0) {
		...
		if (trace < 0) {
			...
			w = synapses_convert_weight_to_input(weight,
