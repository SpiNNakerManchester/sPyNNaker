#include "neuron_izh_curr_impl.h"

#include <debug.h>

// used with file static scope to send input data around
static REAL input_this_timestep;
...
static void neuron_ode(
    ...
    // Update V
    dstateVar_dt[1] = REAL_CONST(140.0)
 ...
 */
static void rk2_kernel_midpoint(REAL h, neuron_pointer_t neuron) {
...
//
neuron_pointer_t create_izh_neuron( REAL A, REAL B, REAL C, REAL D, REAL V,
        REAL U, REAL I) {
    ...
    io_printf(WHERE_TO, "\nA = %11.4k \n", neuron->A);
    neuron->B = B;
    io_printf(WHERE_TO, "B = %11.4k \n", neuron->B);
    neuron->C = C;
    io_printf(WHERE_TO, "C = %11.4k mV\n", neuron->C);
    neuron->D = D;
    io_printf(WHERE_TO, "D = %11.4k ??\n\n", neuron->D);

    neuron->V = V;
    io_printf(WHERE_TO, "V = %11.4k mV\n", neuron->V);
    neuron->U = U;
    io_printf(WHERE_TO, "U = %11.4k ??\n\n", neuron->U);

    neuron->I_offset = I;
    io_printf(WHERE_TO, "I = %11.4k nA?\n", neuron->I_offset);

    neuron->this_h = machine_timestep * REAL_CONST(1.001);
    io_printf(WHERE_TO, "h = %11.4k ms\n", neuron->this_h);
