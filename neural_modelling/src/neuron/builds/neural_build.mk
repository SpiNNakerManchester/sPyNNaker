...
ifndef INPUT_TYPE_H
    INPUT_TYPE_H = $(SOURCE_DIR)/neuron/input_types/input_type_none.h
endif

ifndef NEURON_IMPL_H
	$(error NEURON_IMPL_H is not set.  Please select a neuron implementation header file)
endif

ifndef SYNAPSE_IMPL_H
	$(error SYNAPSE_IMPL_H is not set.  Please select a synapse implementation header file)
endif

ifndef THRESHOLD_TYPE_H
    THRESHOLD_TYPE_H = $(SOURCE_DIR)/neuron/threshold_types/threshold_type_none.h
endif
...
$(TIMING_DEPENDENCE_O): $(TIMING_DEPENDENCE) $(SYNAPSE_TYPE_H) \
                        $(WEIGHT_DEPENDENCE_H) $(SYNAPSE_IMPL_H)
	-mkdir -p $(dir $@)
	$(CC) -D__FILE__=\"$(notdir $*.c)\" -DLOG_LEVEL=$(PLASTIC_DEBUG) $(CFLAGS) \
	        -include $(SYNAPSE_TYPE_H)\
