...
#if LOG_LEVEL >= LOG_DEBUG
static inline bool have_nonempty_ring_buffer(uint32_t n, uint32_t t) {
    for (uint32_t d = 0; d < (1 << SYNAPSE_DELAY_BITS); d++) {
        index_t rb_idx = synapses_get_ring_buffer_index(d + time, t, n,
                synapse_type_index_bits, synapse_index_bits);

        if (ring_buffers[rb_idx] != 0) {
            return true;
        }
    }
    return false;
...
static inline void _print_ring_buffers(uint32_t time) {
    ...
    for (uint32_t n = 0; n < n_neurons; n++) {
        for (uint32_t t = 0; t < n_synapse_types; t++) {
            if (have_nonempty_ring_buffer(n, t)) {
...
static inline void _print_inputs(void) {
#if LOG_LEVEL >= LOG_DEBUG
    log_debug("Inputs\n");
    neuron_print_inputs();
#endif // LOG_LEVEL >= LOG_DEBUG
