...
address_t synaptogenesis_formation_init(address_t data) {
    ...
    form_params = (formation_params *) spin1_malloc(data_size);
    if (form_params == NULL) {
    ...
    ff_probs = &form_params->prob_tables[0];
    lat_probs = &form_params->prob_tables[form_params->ff_prob_size];
