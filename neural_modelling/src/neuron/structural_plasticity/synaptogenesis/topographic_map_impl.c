...
// This macro hugely shortens the rest of the code
#define SUBPOP(idx)	(rewiring_data.pre_pop_info_table.subpop_info[idx])
...
static inline uint32_t uniform_random(void) {
    return mars_kiss64_seed(rewiring_data.local_seed);
}

static inline int my_abs(
	int a) {
    return a < 0 ? -a : a;
}
...
address_t synaptogenesis_dynamics_initialise(
    ...
    for (index = 0; index < rewiring_data.pre_pop_info_table.no_pre_pops; index++) {
        ...
        SUBPOP(index).no_pre_vertices = *half_word++;
        SUBPOP(index).sp_control = *half_word++;
        sp_word = (int32_t *) half_word;
        SUBPOP(index).total_no_atoms = *sp_word++;
        SUBPOP(index).key_atom_info = (int32_t *) sark_alloc(
                KEY_INFO_CONSTANTS * SUBPOP(index).no_pre_vertices,
                sizeof(int32_t));
        for (int32_t j = 0;
                j < KEY_INFO_CONSTANTS * SUBPOP(index).no_pre_vertices;
                j++) {
            SUBPOP(index).key_atom_info[j++] = *sp_word++; // key
            SUBPOP(index).key_atom_info[j++] = *sp_word++; // n_atoms
            SUBPOP(index).key_atom_info[j++] = *sp_word++; // lo_atom
            SUBPOP(index).key_atom_info[j] = *sp_word++;   // mask
        }
    }
...
static inline spike_t select_last_spike(void) {
    ...
    uint32_t offset = ulrbits(uniform_random()) * current_state.no_spike_in_interval;
...
void synaptogenesis_dynamics_rewire(
    ...
    column_offset = ulrbits(uniform_random()) * rewiring_data.s_max;
    ...
    if (!element_exists && !rewiring_data.random_partner) {
        ...
        for (int i=0; i< rewiring_data.pre_pop_info_table.no_pre_pops; i++) {
            ...
            for (int j = 0; j < SUBPOP(i).no_pre_vertices; j++) {
                if ((_spike & SUBPOP(i).key_atom_info[KEY_INFO_CONSTANTS * j + 3])
                	== SUBPOP(i).key_atom_info[KEY_INFO_CONSTANTS * j]) {
                    pre_app_pop = i;
                    pre_sub_pop = subpop_index;
                    choice = _spike & ~ SUBPOP(i).key_atom_info[
			    KEY_INFO_CONSTANTS * subpop_index + 3];
                }
            }
        }
    } else if (!element_exists && rewiring_data.random_partner) {
	pre_app_pop = ulrbits(uniform_random()) * rewiring_data.pre_pop_info_table.no_pre_pops;
	// Select presynaptic subpopulation
	choice = ulrbits(uniform_random()) * SUBPOP(pre_app_pop).total_no_atoms;
	uint32_t sum = 0;
	int i;
	for (i = 0; i < SUBPOP(pre_app_pop).no_pre_vertices; i++) {
	    sum += SUBPOP(pre_app_pop).key_atom_info[KEY_INFO_CONSTANTS * i + 1];
	    if (sum >= choice) {
		break;
	    }
	}
	...
	choice = ulrbits(uniform_random()) * SUBPOP(pre_app_pop).key_atom_info[
	...
	_spike = SUBPOP(pre_app_pop).key_atom_info[
    ...
    current_state.current_controls = SUBPOP(pre_app_pop).sp_control;
    ...
    pre_global_id = SUBPOP(pre_app_pop).key_atom_info[
	    KEY_INFO_CONSTANTS * pre_sub_pop + 2] + current_state.pre_syn_id;
    ...
    log_debug("g_pre_id %d g_post_id %d g_distance_sq %d exists %d %d",
    ...
    while (0 == spin1_dma_transfer(DMA_TAG_READ_SYNAPTIC_ROW_FOR_REWIRING,
	    synaptic_row_address, rewiring_dma_buffer.row, DMA_READ, n_bytes)) {
...
// generate a fake spike and trigger a dma callback and one to be called by
// the dma callback and then call formation or elimination
void synaptic_row_restructure(
	uint dma_id, uint dma_tag) {
    ...
    log_debug("sr_attempt %d %d exists %d",
...
static void inline _dma_enqueue_write(const char *errmsg) {
    while (spin1_dma_transfer(
	    ...
	    rewiring_dma_buffer.n_bytes_transferred) == 0) {
	log_error(errmsg);
    }
}
...
bool synaptogenesis_dynamics_elimination_rule(void) {
    ...
    _dma_enqueue_write("DMA queue full-removal");

    log_debug("\t| RM pre %d post %d # elems %d rec_conn %d @ %d",
...
bool synaptogenesis_dynamics_formation_rule(void) {
    ...
    uint16_t r = ulrbits(uniform_random()) * MAX_SHORT;
    if (r > probability) {
    ...
    _dma_enqueue_write("DMA queue full-formation");

    log_debug("\t| FORM pre %d post %d # elems %d dist %d rec_conn %d @ %d",
	    current_state.global_pre_syn_id,
	    current_state.global_post_syn_id,
	    number_of_connections_in_row(
		    synapse_row_fixed_region(rewiring_dma_buffer.row)),
