...
#include <neuron/synapse_types/synapse_types_dual_excitatory_exponential_impl.h>
#include <neuron/models/neuron_model_lif_error_neuron_impl.h>
...
static bool neuron_impl_do_timestep_update(index_t neuron_index,
    ...
    // 0.001 as trace takes a long time to decay to 0
    if (neuron->local_err > 0.001k) {
    	// only check threshold if label is present
