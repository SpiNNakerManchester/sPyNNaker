...
//	use(changed);
	...
	// which leaves (a), which I guess should be 24 bits to make it up to 32
//	uint32_t record_this = 0;
//	if (added > 0) {
//		uint32_t pre_id = 8000000;
//		uint32_t post_id = 250;
//		uint32_t add = 1;
//		record_this = (add << 0) | (post_id << 1) | (pre_id << 9);
//	} else if (removed > 0) {
//		uint32_t pre_id = 8000;
//		uint32_t post_id = 25;
//		uint32_t add = 0;
//		record_this = (add << 0) | (post_id << 1) | (pre_id << 9);
//	} else {
