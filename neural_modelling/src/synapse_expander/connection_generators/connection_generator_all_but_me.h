...
static bool connection_generator_all_but_me_generate(
    ...
    // Get the ranges to generate within
    ...
    // for now. Note: could also do this for pre-neurons in case ranges don't
    // overlap, but currently just filtering this later.
    uint32_t post_group;
    uint32_t post_value;
    ...
    uint32_t pre_group_start = pre_lo + post_group * obj->n_neurons_per_group;
    ...
    for (uint32_t post = post_start; post <= post_end; post++) {
        ...
        for (uint32_t pre_value = 0; pre_value < obj->n_neurons_per_group;
                pre_value++) {
            ...
            uint32_t pre = pre_group_start + pre_value;

            // Ignore if outside of the pre-range
        ...
        if (post_value == obj->n_neurons_per_group) {
            post_value = 0;
            pre_group_start += obj->n_neurons_per_group;
        }
    }
