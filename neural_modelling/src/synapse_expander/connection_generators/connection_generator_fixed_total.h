...
void *connection_generator_fixed_total_initialise(address_t *region) {
    ...
    struct fixed_total *state = spin1_malloc(sizeof(struct fixed_total));
    // Copy the parameters in
    spin1_memcpy(
            &state->params, params_sdram, sizeof(struct fixed_total_params));
    *region = (address_t) (params_sdram + 1);
    ...
    log_debug(
            ...
            "n potential connections = %u", state->params.allow_self_connections,
            state->params.with_replacement, state->params.n_connections,
            state->params.n_potential_synapses);
    return state;
}
...
uint32_t connection_generator_fixed_total_generate(
    ...
    use(pre_slice_start);
    struct fixed_total *state = (struct fixed_total *) data;

    // If there are no connections left or none to be made, return 0
    if (max_row_length == 0 || state->params.n_connections == 0) {
        return 0;
    }
    ...
    if (pre_neuron_index == (pre_slice_start + pre_slice_count - 1)) {
        n_conns = state->params.n_connections;

    } else {

        // If with replacement, generate a binomial for this row
        if (state->params.with_replacement) {
            n_conns = binomial(
                    state->params.n_connections,
                    state->params.n_potential_synapses, n_values,
                    state->rng);
    ...
    if (state->params.with_replacement) {
        ...
        for (unsigned int i = 0; i < n_conns; i++) {
            uint32_t u01 = (rng_generator(state->rng) & 0x00007fff);
        ...
        for (unsigned int i = n_conns; i < post_slice_count; i++) {
            ...
            const unsigned int u01 = (rng_generator(state->rng) & 0x00007fff);
    ...
    state->params.n_connections -= n_conns;
    state->params.n_potential_synapses -= post_slice_count;
