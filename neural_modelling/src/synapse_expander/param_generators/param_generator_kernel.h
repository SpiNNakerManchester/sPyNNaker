...
typedef struct param_generator_kernel {
...
} param_generator_kernel_t;

struct all_kernel_params {
    param_generator_kernel_t params;
    accum *values;
};
...
static void *param_generator_kernel_initialize(address_t *region) {
    ...
    struct all_kernel_params *params =
            spin1_malloc(sizeof(struct all_kernel_params));
    struct sdram_params *params_sdram = (struct sdram_params *) *region;

    spin1_memcpy(&params->params, &params_sdram->params,
            sizeof(param_generator_kernel_t));
    params->values = params_sdram->values;
    ...
    *region = (address_t) &params_sdram->values[
            params->params.m_kernelHeight * params->params.m_kernelWidth];
    return params;
}
...
static void param_generator_kernel_generate(void *data, uint32_t n_synapses,
    ...
    use(indices);
    struct all_kernel_params *all_params = data;
    param_generator_kernel_t *params = &all_params->params;
    uint16_t pre_c = 0;
