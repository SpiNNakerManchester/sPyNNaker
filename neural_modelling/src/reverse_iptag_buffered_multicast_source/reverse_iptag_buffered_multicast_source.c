...
#define SEQUENCED_EIEIO_PKT 1
#define STOP_PACKET_REQUESTS 2
#define START_PACKET_REQUESTS 3
...
bool multicast_source_data_filled(address_t base_address)
{
  ...
  req.flags = 0x7;
  req.tag = 1; //TODO where does this come from?
  req.dest_port = 0xFF;
...
void sdp_packet_callback(uint mailbox, uint port)
{
  ...
  uint8_t pkt_mode = data_hdr_value >> 14;
