...
#include "implementations/meanfield_impl.h"
...
void neuron_do_timestep_update(timer_t time, uint timer_count) { // EXPORTED
    ...
    for (index_t neuron_index = 0; neuron_index < n_neurons; neuron_index++) {
        ...
        if (spike) {
            log_debug("meanfield %u reinit at time %u", neuron_index, time);
            ...
            log_debug("the neuron %d has been determined to not reinit",
                      neuron_index);
         }
    }
...
void neuron_add_inputs( // EXPORTED
    ...
    neuron_impl_add_inputs(synapse_type_index,
            neuron_index, weights_this_timestep);
}
...
const char *neuron_get_synapse_type_char(uint32_t n_neurons){//synapse_type) { // EXPORTED
