...
state_t error_function( REAL x, REAL factor, mathsbox_t *restrict mathsbox){ 
//devra coder fonction powerof 
    ...
    for(t=0; t=x; t+=dt){
        Erfc +=  factor*(2/sqrtk(Pi))*expk(-(t*t));
    }
...
state_t threshold_func(config_t *restrict config)
{
    /*
...
state_t get_fluct_regime_varsup(REAL Ve, REAL Vi, config_t *restrict params)
{
    REAL Fe;
    REAL Fi;
    REAL muGe, muGi, muG;
...
state_t TF(REAL Ve, REAL Vi, meanfield_t *meanfield, config_t *restrict config, mathsbox_t *restrict mathsbox){
...
*/
    ...
    factor = REAL_HALF(config->Gl/(config->TvN * config->Cm));
