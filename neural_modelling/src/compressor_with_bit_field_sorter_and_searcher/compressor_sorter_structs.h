...
typedef struct bf_by_processor_t{
    // processor id
...
typedef struct proc_cov_by_bitfield_t{
    // processor id
    uint32_t processor_id;
    // length of the list
    uint32_t length_of_list;
...
typedef struct address_region_data_t{
    ...
    bit_field_region_t* bf_region;
    // the address of the key atom map
    key_atom_data_t* incoming_key_atom_map_address;
