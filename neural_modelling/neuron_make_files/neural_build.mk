...
BUILD_DIR = $(NEURAL_MODELLING_DIRS)/build/$(APP)/
...
ifndef NEURON_MODEL
    ...
    NEURON_MODEL_O := $(patsubst $(NEURON_MODIFIED_DIR)%.c,$(BUILD_DIR)%.o,$(NEURON_MODEL))
endif
...
ifdef WEIGHT_DEPENDENCE
    ...
    WEIGHT_DEPENDENCE_O := $(patsubst $(NEURON_MODIFIED_DIR)%.c,$(BUILD_DIR)%.o,$(WEIGHT_DEPENDENCE))
endif

ifdef TIMING_DEPENDENCE
    TIMING_DEPENDENCE := $(NEURON_MODIFIED_DIR)$(TIMING_DEPENDENCE)
    TIMING_DEPENDENCE_O := $(patsubst $(NEURON_MODIFIED_DIR)%.c,$(BUILD_DIR)%.o,$(TIMING_DEPENDENCE))
endif
...
OBJECTS := $(patsubst $(NEURON_MODIFIED_DIR)%.c,$(BUILD_DIR)%.o,$(SOURCES))
OBJECTS := $(patsubst $(COMMON_MODIFIED_DIR)%.c,$(BUILD_DIR)%.o,$(OBJECTS))
...
# Simple build rules
$(BUILD_DIR)%.o: $(COMMON_MODIFIED_DIR)%.c $(C_FILES_MODIFIED)
	# Simple
	# $@ 
	# $<
	# $(CFLAGS)
...
$(BUILD_DIR)%.o: $(NEURON_MODIFIED_DIR)%.c $(C_FILES_MODIFIED)
	# Simple
	-mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -o $@ $<
...
$(BUILD_DIR)c_main.o: $(NEURON_MODIFIED_DIR)c_main.c $(C_FILES_MODIFIED)
	#c_main.c
	-mkdir -p $(dir $@)
	$(SYNAPSE_TYPE_COMPILE) -o $@ $<

$(BUILD_DIR)synapses.o: $(NEURON_MODIFIED_DIR)synapses.c $(C_FILES_MODIFIED)
	#synapses.c
	-mkdir -p $(dir $@)
	$(SYNAPSE_TYPE_COMPILE) -o $@ $<

$(BUILD_DIR)spike_processing.o: $(NEURON_MODIFIED_DIR)spike_processing.c $(C_FILES_MODIFIED)
	#spike_processing.c
	-mkdir -p $(dir $@)
	$(SYNAPSE_TYPE_COMPILE) -o $@ $<

$(BUILD_DIR)population_table/population_table_fixed_impl.o: $(NEURON_MODIFIED_DIR)population_table/population_table_fixed_impl.c $(C_FILES_MODIFIED)
	#population_table/population_table_fixed_impln.c
	-mkdir -p $(dir $@)
	$(SYNAPSE_TYPE_COMPILE) -o $@ $<

$(BUILD_DIR)population_table/population_table_binary_search_impl.o: $(NEURON_MODIFIED_DIR)population_table/population_table_binary_search_impl.c $(C_FILES_MODIFIED)
...
STDP_ENABLED = 0
SYNAPSE_DYNAMICS_O := $(patsubst $(NEURON_MODIFIED_DIR)%.c,$(BUILD_DIR)%.o,$(SYNAPSE_DYNAMICS))
SYNAPTOGENESIS_DYNAMICS_O := $(patsubst $(NEURON_MODIFIED_DIR)%.c,$(BUILD_DIR)%.o,$(SYNAPTOGENESIS_DYNAMICS))

ifneq ($(SYNAPSE_DYNAMICS), $(NEURON_MODIFIED_DIR)plasticity/synapse_dynamics_static_impl.c)
    ifdef TIMING_DEPENDENCE_H
	TIMING_DEPENDENCE_H := $(NEURON_MODIFIED_DIR)$(TIMING_DEPENDENCE_H)
    else
        $(error TIMING_DEPENDENCE_H is not set which is required when SYNAPSE_DYNAMICS != plasticity/synapse_dynamics_static_impl.c)
    endif
    ifdef WEIGHT_DEPENDENCE_H
	WEIGHT_DEPENDENCE_H := $(NEURON_MODIFIED_DIR)$(WEIGHT_DEPENDENCE_H)
    else
        $(error WEIGHT_DEPENDENCE_H is not set which is required when SYNAPSE_DYNAMICS != plasticity/synapse_dynamics_static_impl.c)
    endif
    ...
    $(BUILD_DIR)/plasticity/common/post_events.o: $(NEURON_MODIFIED_DIR)/plasticity/common/post_events.c
