...
class TestSampleProfiling(BaseTestCase):
    ...
    def assert_report(self) -> None:
        ...
        with open(path) as report:
            report_contents = json.load(report)
            assert isinstance(report_contents["simulation_timestep_us"], int)
            assert isinstance(report_contents["hardware_timestep_us"], int)
