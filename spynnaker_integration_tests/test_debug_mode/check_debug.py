...
class CheckDebug(BaseTestCase):
    ...
    def _load_reports(self, run_number):
        return [
            reports_names._C_ROUTING_TABLE_DIR.format(run_number),
            reports_names._COMPARED_FILENAME.format(run_number),
            # write_routing_tables_from_machine_report
            routing_tables_from_machine_report.format(run_number),
            # write_memory_map_report
            memory_map_on_host_report.format(run_number),
            "ds{}.sqlite3".format(run_number),
        ]

    def _run_reports(self, run_number):
        return [
            # write_drift_report_end or start
            CLOCK_DRIFT_REPORT.format(run_number),
            "data{}.sqlite3".format(run_number),
            ]
    ...
    def debug(self):
        ...
        pop.get_data("v")
        sim.run(10)  # third run
        found = os.listdir(SpynnakerDataView.get_run_dir_path())
        for report in self._run_reports("3"):
            self.assertIn(report, found)

        sim.reset()  # soft with dsg
        SpynnakerDataView.set_requires_data_generation()
        sim.run(10)  # fourth run
        pop.get_data("v")
        self.assertEqual(run0, SpynnakerDataView.get_run_dir_path())
        found = os.listdir(run0)
        for report in self._load_reports("4"):
            self.assertIn(report, found)
        for report in self._run_reports("4"):
            self.assertIn(report, found)
        # No point in checking files they are already there
