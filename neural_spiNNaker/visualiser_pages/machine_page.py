...
from SpiNNaker.neural_spiNNaker.visualiser_pages.chip_page import ChipPage
...
class MachinePage(AbstractPage):
    ...
    def __init__(self, static, scope, x_dim, y_dim, machine, placements):
    ...
    def _set_up_chips_in_machine(self, scope):
        ...
        for x in range(x_dim):
            for y in range(y_dim):
                ...
                column_y = self._correct_y_pos(y, self.y_dim)
                ...
                if scope == MachinePage.CHIP_SCOPE:
                    ...
                    self.button_mapping[menu] = {'x': x, 'y': y, 'b': button}
                    ...
                    if self.dao.machine.chip_exists_at(x, y):
                        ...
                        self.machine_table.attach(core_table, column_x,
                                                  column_x+1, column_y,
                                                  column_y+1)
                        ...
                        for core_id in core_ids:
                            if core_id is None:
                                ...
                                self.button_mapping[key] = {'x': x, 'y': y,
                                                            'b': button}
    ...
    def _button_press(self, widget, event):
        ...
        if event.type == gtk.gdk.BUTTON_PRESS and event.button == 1:
            ...
            self.chips_with_views[chip_coords[0]+chip_coords[1]] = ChipPage
            ChipPage(self.machine.get_chip_at_location({'x': chip_coords[0],
                                                        'y': chip_coords[1]},
                     self.placements.get_placment_by_chip(chip_coords)))
        return False
    ...
    def _add_edges(self):
        ...
        for x in range(x_dim):
            for y in range(y_dim):
                if self.machine.chip_exists_at_xy(x, y):
                    index = 0
                    chip = self.machine.get_chip(x, y)
                    for connection in chip.router.neighbourlist:
                        #calculate edge position
                        column_y = \
                            self._correct_y_pos(y, self.y_dim) + \
                            adjustments[index]['y']
                        column_x =\
                        ...
                        if connection is not None:
                            if index == 0:  # E, W
                                self.machine_table.attach(gtk.Label("-"),
                                                          column_x, column_x+1,
                                                          column_y, column_y+1)
                            if index == 1:  # ne sw
                                self.machine_table.attach(gtk.Label("/"),
