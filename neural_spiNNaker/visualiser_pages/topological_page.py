...
class TopologicalPage(AbstractPage):
    def __init__(self, vertex_in_question, retina_drop_off_theshold,
                 coord_to_low_atom_mapping):
        AbstractPage.__init__(self)

        self.rectangle_size = {'x': 1, 'y': 1}
        self.exposure_bar_size = 20
        self.exposure_bar_label_size = 40
        self.exposure_bar_space_size = 20
        self.legend_size = {'x': 40, 'y': 20}
        self.no_blocks_in_exposure_plot = 200
        self.no_labels = 10
        self.no_color_mapping_labels = 10
        self.pixmap = None
        ...
        self._coord_to_low_atom_mapping = coord_to_low_atom_mapping
    ...
    def _generate_objects(self):
        ...
        """
        max_y_pos = \
            (self._y_dim * self.rectangle_size['y']) - self.legend_size['x']
    ...
    def _generate_retina_view(self):
        ...
        to be a size shaped)
        """
        height_of_screen = self._y_dim * self.rectangle_size['y']
        width_of_screen = self._x_dim * self.rectangle_size['x']
        ##add exposureBar
    ...
    def _add_x_y_axis_labels(self, cr):
        height_of_screen = self._y_dim * self.rectangle_size['y']
        y_position = math.floor(self._y_dim / self.no_labels)
        x_position = math.floor(self._x_dim / self.no_labels)
    ...
    def _size_allocate(self, requisition):
        ...
        if len(self._objects_to_draw.keys()) != 0:
            ...
            y = math.floor((height - self.legend_size['y']) / self._y_dim)
            self.rectangle_size = {'x': x, 'y': y}
            #go through drawing objects updating new x,y width and height
            max_y_pos = self._y_dim * self.rectangle_size['y']
            for x in range(self._x_dim):
                for y in range(self._y_dim):
                    x_pos = ((x * self.rectangle_size['x']) +
                             self.legend_size['x'])
                    y_pos = y * self.rectangle_size['y']
                    width = self.rectangle_size['x']
                    height = self.rectangle_size['y']
    ...
    def _convert_pop_spike_to_x_y(self, spike_word):
        ...
        lo_atom = self._coord_to_low_atom_mapping("{}:{}:{}".format(x, y, p))
