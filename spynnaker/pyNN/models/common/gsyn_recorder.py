        ...
        if len(self._missing_str) > 0:
            logger.warn(
                ...
                " from the following cores: {}".format(
                    label, region, self._missing_str))
        data = numpy.vstack(data)
        order = numpy.lexsort((data[:, 1], data[:, 0]))
        return data[order]

    def _get_gsyn_for_vertex(self, vertex, graph_mapper, placements,
        ...
        if data_missing:
            ...
            y = placement.y
            p = placement.p
            self._missing_str += "({}, {}, {}); ".format(x, y, p)
        ...
        return numpy.reshape(part_data, [-1, 4])
