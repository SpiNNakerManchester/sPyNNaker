...
from spynnaker.pyNN.models.abstract_models\
    .abstract_provides_keys_and_masks_vertex import \
...
class CommandSender(AbstractMultiCastSource,
    ...
    def _calculate_memory_requirements(self):
        ...
        for start_command in commands:
            ...
            # if first command, initialize counter
            if len(commands_in_same_time_slot) == 0:
                ...
                if commands_in_same_time_slot[0]['t'] == start_command['t']:
                    ...
                    # if not, then send all previous messages to
                    ...
                    # 3 ints holding coutners of cp, cnp and t
                    self._memory_requirements += 12
                    self._memory_requirements += self.size_of_message(
                        start_command)
                    self._writes.append(start_command['t'])
    ...
    def add_commands(self, commands, commands_key, commands_mask, edge):
        self._edge_map[edge] = (commands_key, commands_mask)
    ...
    def get_keys_and_masks_for_partitioned_edge(self, partitioned_edge,
        ...
        if edge in self._edge_map:
            ...
            raise Exception("An unrecognised edge {} appears to come out of "
                            "the command sender!".format(edge))
                    ...
                    if combo is None:
                        ...
                        if combo != new_combo:
                            raise exceptions.RallocException(
                                "The keys going down a specific subedge are"
                                " not consistent")
