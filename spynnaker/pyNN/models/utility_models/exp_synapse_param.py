...
def write_exp_synapse_param(tau, machine_time_step, n_atoms, spec):
    ...
    time_step = numpy.divide(-float(machine_time_step), numpy.multiply(1000.0, tau))
    decay = numpy.exp(time_step)
    init = numpy.multiply(tau, numpy.subtract(1.0, decay))
    ...
    # Otherwise, if we have generated decays and inits for each atom
        ...
        raise exceptions.SynapticBlockGenerationException(str("Cannot generate synapse parameters from %u values" % rescaled_decay.size))