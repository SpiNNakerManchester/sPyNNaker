...
class WeightBasedPlasticSynapseRowIo(AbstractSynapseRowIo):
    ...
    def get_packed_fixed_plastic_region(synapse_row, weight_scale,
        ...
        if max(synapse_row.delays) > max_delay:
    ...
    def get_packed_plastic_region(self, synapse_row, weight_scale,
        ...
        abs_scaled_weights = numpy.asarray(numpy.abs(synapse_row.weights)
                                           * weight_scale, dtype='uint16')

        # As we're packing into uint32s, add extra weight if we have an odd
        # number
        if (len(abs_scaled_weights) % 2) != 0:
            abs_scaled_weights = numpy.asarray(numpy.append(abs_scaled_weights,
                                                            0), dtype='uint16')

        # Create view of weights as uint32s

        # noinspection PyNoneFunctionAssignment
        abs_scaled_weights_view = \
            abs_scaled_weights.view(dtype='uint32')
