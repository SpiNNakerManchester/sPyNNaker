...
class PfisterSpikeTripletTimeDependence(AbstractTimeDependency):
    ...
    def write_plastic_params(self, spec, machine_time_step, weight_scale):
        ...
        # Write lookup tables
        plasticity_helpers.write_exponential_decay_lut(spec, self._tau_plus,
                                                 LOOKUP_TAU_PLUS_SIZE,
                                                 LOOKUP_TAU_PLUS_SHIFT)
        plasticity_helpers.write_exponential_decay_lut(spec, self._tau_minus,
                                                 LOOKUP_TAU_MINUS_SIZE,
                                                 LOOKUP_TAU_MINUS_SHIFT)
        plasticity_helpers.write_exponential_decay_lut(spec, self._tau_x,
                                                 LOOKUP_TAU_X_SIZE,
                                                 LOOKUP_TAU_X_SHIFT)
        plasticity_helpers.write_exponential_decay_lut(spec, self._tau_y,
                                                 LOOKUP_TAU_Y_SIZE,
                                                 LOOKUP_TAU_Y_SHIFT)