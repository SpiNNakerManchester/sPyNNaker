...
class SpikeSourcePoissonMachineVertex(
    ...
    def _write_poisson_rates(self, spec, placement):
        ...
        for (start, stop, data) in data_slice.iter_ranges():
            count = stop - start
            items = numpy.dstack(
                (DataType.S1615.encode_as_numpy_int_array(data['rates']),
                 DataType.S1615.encode_as_numpy_int_array(data['starts']),
                 DataType.S1615.encode_as_numpy_int_array(data['durations']))
                )[0]
            data_items.extend([[count], [len(items)], [0], numpy.ravel(items)])
            n_items += 1
