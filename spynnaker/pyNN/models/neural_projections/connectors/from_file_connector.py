        ...
        for line in file_lines:
            ...
            if form1:
               ...
               if form2:
                  ...
                  form3 = re.match(r"((?:\[[^[\]\s]+\])|(?:[^[\],\s]+))((?:,\s*)"
                                   +r"|\s+)((?:\[[^[\]\s]+\])|(?:[^[\],\s]+))"
                                   +r"((?:,\s*)|\s+)((?:\+|\-)?\d+(?:\.\d+)?"
                                   +r"(?:(?:E|e)(?:\+|\-)?\d+)?)((?:,\s*)|\s+)"
                                   +r"((?:\+|\-)?\d+(?:\.\d+)?(?:(?:E|e)"
                                   +r"(?:\+|\-)?\d+)?)\s*", line)
                  #form 3: a comma- or space-separated set of specifiers per line
                  if form3:
                     conn_sublist = [(eval(form3.group(1)), eval(form3.group(3)), 
                                   float(form3.group(5)), float(form3.group(7)))]
                  else:
                     raise exceptions.ConfigurationException(
                           "Invalid connection file format")
            self._conn_list.extend(conn_sublist) 
        return FromListConnector.generate_synapse_list(self, 
