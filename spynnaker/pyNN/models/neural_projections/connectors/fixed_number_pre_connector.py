...
class FixedNumberPreConnector(AbstractGenerateConnectorOnMachine):
    ...
    def set_projection_information(self, rng, machine_time_step, synapse_info):
        ...
        if (not self.__with_replacement and
                self.__n_pre > self.n_pre_neurons(synapse_info)):
    ...
    def _get_pre_neurons(self, synapse_info):
        ...
        if not self.__pre_neurons_set:
            ...
            for m in range(0, self.n_post_neurons(synapse_info)):
                ...
                if (self.pre_population(synapse_info) is self.post_population(
                    ...
                    # list
                    no_self_pre_neurons = [
                        n for n in range(self.n_pre_neurons(
                            synapse_info)) if n != m]
                    ...
                    self.__pre_neurons[m] = self._rng.choice(
                        self.n_pre_neurons(synapse_info), self.__n_pre,
                        self.__with_replacement)
    ...
    def get_n_connections_from_pre_vertex_maximum(
        ...
        prob_selection = 1.0 / float(self.n_pre_neurons(synapse_info))
        n_connections_total = utility_calls.get_probable_maximum_selected(
            self.n_pre_neurons(
                synapse_info) * self.n_post_neurons(synapse_info),
            self.__n_pre * self.n_post_neurons(synapse_info), prob_selection,
            chance=1.0/10000.0)
        prob_in_slice = (
    ...
    def get_weight_maximum(self, weights, synapse_info):
        ...
        return self._get_weight_maximum(
            weights, self.__n_pre * self.n_post_neurons(synapse_info))
