...
class IFCurrExpSEMDBase(AbstractPyNNNeuronModelStandard):
    ...
    def __init__(
            ...
            multiplicator=0.0, inh_input_previous=None):
        if v is not None:
        ...
        if isyn_inh is not None:
            logger_utils.warn_once(
                logger, "Formal Pynn specifies that 'isyn_inh' should be set "
                        "using initial_values = not cellparams")
        if inh_input_previous is not None:
            logger_utils.warn_once(
                logger, "Formal Pynn specifies that 'inh_input_previous' "
                        "should be set using initial_values = not cellparams")
