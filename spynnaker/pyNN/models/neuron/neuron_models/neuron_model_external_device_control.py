...
class NeuronModelExternalDeviceControl(AbstractNeuronModel):
    """ Classic leaky integrate and fire neuron model but augmented to stream
    voltage instead of spikes. Used primarily for external devices.
    """
    __slots__ = [
        "__v_init",
        "__v_rest",
    ...
    def __init__(
        ...
        """
        super(NeuronModelExternalDeviceControl, self).__init__(
            [DataType.S1615,   # v
        ...
        return [state_variables[V], parameters[V_REST],
                ...
                parameters[TAU_M].apply_operation(
                    operation=lambda x: numpy.exp(
                        float(-ts) / (MICRO_TO_MILLISECOND_CONVERSION * x))),
                parameters[I_OFFSET], state_variables[COUNT_REFRAC],
                parameters[V_RESET],
                parameters[TAU_REFRAC].apply_operation(
                    operation=lambda x: int(numpy.ceil(
                        x / (ts / MICRO_TO_MILLISECOND_CONVERSION))))]
