...
class MasterPopTableAsBinarySearch(object):
    ...
    def get_next_allowed_address(next_address):
        ...
        """
        next_address = (
            (next_address + (_ADDRESS_SCALE - 1)) //
            _ADDRESS_SCALE) * _ADDRESS_SCALE
    ...
    def extract_synaptic_matrix_data_location(
            cls, incoming_key, master_pop_base_mem_address, txrx,
            chip_x, chip_y):
        """
        ...
        entry = cls._locate_entry(entry_list, incoming_key)
        if entry is None:
            return []
        addresses = list()
        for i in range(entry["start"], entry["start"] + entry["count"]):
            address_word = address_list[i]
            is_single = (address_word & _SINGLE_BIT_FLAG_BIT) > 0
            address = address_word & _ADDRESS_MASK
            row_length = address_word & _ROW_LENGTH_MASK
            if is_single:
                address = address >> 8
            else:
