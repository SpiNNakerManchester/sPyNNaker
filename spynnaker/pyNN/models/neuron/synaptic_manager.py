...
# 1 word for n_edges
# 2 words for post_vertex_slice.lo_atom, post_vertex_slice.n_atoms
# 1 word for n_synapse_types
# 1 word for n_synapse_type_bits
# 1 word for n_synapse_index_bits
    ...
    def _write_synaptic_matrix_and_master_population_table(
                ...
                for synapse_info in app_edge.synapse_information:
                    ...
                    if (isinstance(
                            ...
                            not isinstance(
                                ...
                                AbstractSynapseDynamicsStructural)):
                        generate_on_machine.append(_GenOnMachine(
        ...
        for gen_data in generate_on_machine:
            block_addr, index = self.__generate_on_chip_data(
                ...
                generator_data)
            key = (gen_data.synapse_info, gen_data.pre_slice.lo_atom,
                   post_vertex_slice.lo_atom)
            self.__synapse_indices[key] = index
    ...
    def __generate_on_chip_data(
        ...
        if (max_row_info.delayed_max_n_synapses > 0 and
            ...
            gen.app_edge.delay_edge.pre_vertex.add_generator_data(
                ...
                gen.pre_slices, gen.pre_index, post_slices,
                post_slice_index, gen.pre_slice, post_vertex_slice,
                gen.synapse_info, gen.app_edge.n_delay_stages + 1,
                machine_time_step)
        elif max_row_info.delayed_max_n_synapses != 0:
            raise Exception(
                "Found delayed items but no delay "
                "machine edge for {}".format(gen.app_edge.label))
        ...
        if max_row_info.undelayed_max_n_synapses:
            ...
            n_bytes_undelayed = (max_row_info.undelayed_max_bytes *
                                 gen.pre_slice.n_atoms)
        ...
        delay_rinfo = None
        n_delay_stages = 0
        delay_key = (gen.app_edge.pre_vertex, gen.pre_slice.lo_atom,
                     gen.pre_slice.hi_atom)
        ...
        generator_data.append(GeneratorData(
            ...
            gen.pre_index, post_slices, post_slice_index,
            gen.pre_slice, post_vertex_slice, gen.synapse_info,
            n_delay_stages + 1, machine_time_step))
