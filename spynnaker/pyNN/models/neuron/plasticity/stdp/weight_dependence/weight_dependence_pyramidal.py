...
class WeightDependencePyramidal(AbstractWeightDependence):
    __slots__ = [
        "__w_max",
        "__w_min",
        "__learning_rates"]
    ...
    def is_same_as(self, weight_dependence):
        ...
        if not isinstance(weight_dependence, WeightDependencePyramidal):
            return False
        return (
            (self.__w_min == weight_dependence.w_min) and
            (self.__w_max == weight_dependence.w_max) and
            (self.__learning_rates == weight_dependence.learning_rates))
    ...
    def write_parameters(
        ...
        for w in range(len(weight_scales)):
            spec.write_value(
                data=int(round(self.__w_min * weight_scales[w])), data_type=DataType.INT32)
            spec.write_value(
                data=int(round(self.__w_max * weight_scales[w])), data_type=DataType.INT32)

            spec.write_value(
                data=self.__learning_rates[w], data_type=DataType.S1615)
    ...
    def get_parameter_names(self):
        return ['w_min', 'w_max', 'learning_rates']
