...
def print_summary(profiling_data, duration):
    ...
    for tag_name, times in profiling_data.iteritems():
        ...
        print("\tStandard error:%fms" % (np.std(times[1]) /
                                         np.sqrt(times[1].size)))

        # Digitise the sample entry times into these bins
...
def subtract_tag(profile_data, tag, subtract_tag):
    ...
    for i, (entry_time, exit_time, duration) in enumerate(
            zip(profile_data[tag][0], exit_times, profile_data[tag][1])):
        # Bisect to find range of subtract tags within tag
        first_subtract_exit = bisect.bisect_right(
            subtract_exit_times, entry_time)
        last_subtract_entry = bisect.bisect_left(
            subtract_entry_times, exit_time)

        # Extract this slice of subtract tag durations
        durations_to_subtract = profile_data[subtract_tag][1][
            first_subtract_exit:last_subtract_entry]
...
def time_filter_core_profiling_data(profile_data, min_time, max_time):
    ...
    for tag, times in profile_data.iteritems():
        ...
        # Add entry times and durations, filtered by new indices to filtered
        # tag dictionary
        filtered_tag_dictionary[tag] = (
            times[0][filtered_indices], times[1][filtered_indices])
