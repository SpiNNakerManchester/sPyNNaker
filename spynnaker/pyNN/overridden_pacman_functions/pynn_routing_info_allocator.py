        ...
        if len(router_constraints) == 0:
            ...
            if len(fixed_mask_constraints) != 0:
                if len(fixed_mask_constraints) > 1:
                    raise exceptions.PacmanRouteInfoAllocationException(
                        "cannot determine how to reduce more than one fixed "
                        "mask constraint please reduce the constraints and "
                        "try again, or use another routing info allocator")
                if mask != router_constraints[0].mask:
                    raise exceptions.PacmanRouteInfoAllocationException(
                        "the mask ({}) determined by your key-mask function "
                        "does not match the mask ({}) used in the fixed mask "
                        "constraint. Please rectify your constraints and try "
