...
class OnChipBitFieldGenerator(object):
    ...
    def __call__(
        ...
        :param generating_bit_field_summary_report:
            bool flag for making summary report
        :rtype: None
        """
    ...
    def _read_back_and_summarise_bit_fields(
            self, app_graph, transceiver, placements, default_report_folder):
            ...
            for app_vertex in progress.over(app_graph.vertices):
                ...
                for vertex in app_vertex.machine_vertices:
                    if isinstance(vertex, AbstractSupportsBitFieldGeneration):
                        self.__summarise_one(
                            vertex, placements.get_placement_of_vertex(vertex),
                            transceiver, chip_packet_count,
                            chip_redundant_count, output)
            ...
            for (x, y) in chip_packet_count:
                output.write(
                    ...
                    "percentage of {} \n".format(
                        ...
                        _percent(chip_redundant_count[x, y],
                                 chip_packet_count[x, y])))
    ...
    def __summarise_one(
            self, vertex, placement, txrx, total_count, redundant_count,
            output):
        local_total = 0
        local_redundant = 0
        ...
        for _bit_field_index in range(0, n_bit_field_entries):
            ...
            bit_field = struct.unpack(
                ...
                txrx.read_memory(
                    placement.x, placement.y, bitfield_pointer,
            ...
            for neuron_id in range(0, n_neurons):
                if self._bit_for_neuron_id(bit_field, neuron_id) == 0:
                    ...
                    local_redundant += 1
                total_count[placement.x, placement.y] += 1
                local_total += 1
    ...
    def __write_single_core_data(
            self, vertex, placements, txrx, bit_field_expander_path,
            expander_cores):
        placement = placements.get_placement_of_vertex(vertex)
    ...
    def _check_for_success(self, executable_targets, transceiver):
        ...
        for core_subset in executable_targets.all_core_subsets:
            for p in core_subset.processor_ids:
                # Read the result from USER2 register
                result = self.__get_status(core_subset, p, transceiver)
    ...
    def __get_status(self, core_subset, p, transceiver):
        """ Read the result from USER2 register """
        x = core_subset.x
        y = core_subset.y
