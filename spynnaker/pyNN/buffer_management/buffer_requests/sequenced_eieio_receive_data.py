...
from spynnaker.pyNN.buffer_management.buffer_requests.abstract_eieio_packets.\
    abstract_eieio_command_packet import AbstractEIEIOCommandPacket


class SequencedEIEIOReceiveData(AbstractEIEIOCommandPacket):

    def __init__(self, chip_x, chip_y, chip_p, address_pointer, size):
        AbstractEIEIOCommandPacket.__init__(self)
        self._chip_x = chip_x
        self._chip_y = chip_y
        self._chip_p = chip_p
    ...
    def get_eieio_message_as_byte_array(self):
        header = EIEIOCommandHeader(
            spinnman_constants.EIEIO_COMMAND_IDS.READ_DATA.value)
