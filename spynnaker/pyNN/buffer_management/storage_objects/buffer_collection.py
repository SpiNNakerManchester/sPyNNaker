...
class BufferCollection(object):
    ...
    def is_region_empty(self, region_id):
        ...
        if self.is_buffers_to_use_for_region(region_id):
            buffers_to_send_empty = self._buffers_to_use[region_id].\
                is_region_empty()
    ...
    def add_sent_packet(self, packet, region_id):
        if region_id not in self._buffers_sent.keys():
            # the sent packet deque should be created when the set of buffers
            # in firstly inserted
            raise exceptions.BufferedRegionNotPresent(
                "Trying to queue packet for a unmanaged region")
    ...
    def add_sent_packets(self, packets, region_id):
        if isinstance(packets, list):
            ...
            # error in call parameter: packets needs to be a list
            raise exceptions.InvalidParameterType(
                "Invalid parameter type in add_sent_packets call")

    def remove_packets_in_region_in_seq_no_interval(self, region_id,
                                                    min_seq_no, max_seq_no):
        if self.is_region_managed(region_id):
            self._buffers_sent[region_id].remove_packets_in_seq_no_interval(
                min_seq_no, max_seq_no)
        else:
            # the region is not managed
            raise exceptions.BufferedRegionNotPresent(
                "Trying to access buffer packets in a unmanaged region")
